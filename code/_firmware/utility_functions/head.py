...
class Head:
    ...
    def set_head_theta(self, roll, yaw):
        ...
        if self.current_thetas != self.last_thetas:
            self.pca_obj.set_servo_angle(self.pins[0], yaw)  # Example: Set camera angle based on roll
            self.pca_obj.set_servo_angle(self.pins[1], roll)  # Example: Set camera angle based on roll
