...
def plot_foot_trajectories(left_pos, right_pos, movement_name, out_dir="."):
    ...
    #ax1[1, 0].set_ylim(lims_y)
    #ax1[1, 1].set_ylim(lims_y)
    ax1.grid(True)
    ax1.legend()
    ...
    # 2) Y vs Z (plan / overhead)
    fig2, ax2 = plt.subplots(figsize=(8, 6))
    ax2.plot(left_pos[:, 1], left_pos[:, 2], '-o', markersize=3, label="Left Foot")
    ax2.plot(right_pos[:, 1], right_pos[:, 2], '-o', markersize=3, label="Right Foot")
    ax2.set_xlabel("Y (units)")
    ax2.set_ylabel("Z (units)")
    ax2.set_title(f"Foot Trajectories (Y vs Z) — {movement_name}")
    ax2.grid(True)
    ax2.legend()
    out2 = os.path.join(out_dir, f"{movement_name}_foot_YZ.png")
    fig2.savefig(out2)
    print(f"Saved Y-Z trajectory to {out2}")
